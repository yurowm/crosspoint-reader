...
int BaseTheme::getListPageItems(const int contentHeight, const bool hasSubtitle) const {
  const int rowStep = getListRowStep(hasSubtitle);
  return rowStep > 0 ? std::max(1, contentHeight / rowStep) : 1;
}

void BaseTheme::drawList(const GfxRenderer& renderer, const Rect rect, const int itemCount, const int selectedIndex,
  ...
  if (totalPages > 1) {
    ...
    for (int i = 0; i < arrowSize; ++i) {
      ...
      renderer.drawLine(centerX - i, rect.y + i, centerX - i + lineWidth - 1, rect.y + i);
    }
    for (int i = 0; i < arrowSize; ++i) {
  ...
  for (int i = pageStartIndex; i < itemCount && i < pageStartIndex + pageItems; ++i) {
    ...
    if (rowValue != nullptr) {
      ...
      if (!valueText.empty()) {
        const int maxValueWidth = std::max(0, rowTextWidth - 40 - minValueGap);
    ...
    const std::string item = renderer.truncatedText(UI_10_FONT_ID, itemName.c_str(), rowTextWidth);
    const int textX = rect.x + BaseMetrics::values.contentSidePadding;
    renderer.drawText(UI_10_FONT_ID, textX, itemY, item.c_str(), i != selectedIndex);
    if (rowDimmed && rowDimmed(i) && i != selectedIndex) {
      const int titleWidth = renderer.getTextWidth(UI_10_FONT_ID, item.c_str());
      const int lineHeight = renderer.getLineHeight(UI_10_FONT_ID);
      for (int y = itemY; y < itemY + lineHeight; ++y) {
    ...
    if (rowSubtitle != nullptr) {
      ...
      if (!subtitleText.empty()) {
        const std::string subtitle = renderer.truncatedText(SMALL_FONT_ID, subtitleText.c_str(), rowTextWidth);
    ...
    if (!valueText.empty()) {
      ...
      const int valueY = itemY + (rowSubtitle != nullptr ? 10 : 0);
      renderer.drawText(UI_10_FONT_ID, rect.x + contentWidth - BaseMetrics::values.contentSidePadding - valueWidth,
...
void BaseTheme::drawStatusBar(GfxRenderer& renderer, const float bookProgress, const int currentPage,
  ...
  if (BoardConfig::isX4Pro()) {
    ...
    const int rightWidth = renderer.getTextWidth(SMALL_FONT_ID, pageText);
    renderer.drawText(SMALL_FONT_ID, screenWidth - horizontalPadding - rightWidth, bottomTextY, pageText,
                      panelTextBlack);
